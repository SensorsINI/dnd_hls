...
#pragma HLS ARRAY_PARTITION variable=timestamp_polarity_image cyclic factor=1 dim=2
//#pragma HLS INLINE
#ifdef VERBOSE
...
//
//			ts_pol = timestamp_polarity_image[TI_addr];															// Get next data from the TPI
...
//	timestamp_polarity_image[spike_y * DVS_WIDTH + spike_x] = timestamp_polarity;						// Update the TPI with the current spike information (ts and pol) in the address corresponding to the current spike
	...
	// DIFFERENT MEMORY INTERFACE. FROM HERE -------------------------------------------
	timestamp_polarity_image_data_t *whole_colum;
	timestamp_polarity_image_data_t combined_spike;
...
#pragma HLS UNROLL
			...
			if(t_p_idx == (COMBINED_SPIKES - 1)){															// Check if the combined spike limit is reached
				combined_spike = timestamp_polarity_image[spike_surround_x+patch_x][spike_combined_idx+1];			// Get the next combined spike data
	...
	// DIFFERENT MEMORY INTERFACE. TO HERE -------------------------------------------
