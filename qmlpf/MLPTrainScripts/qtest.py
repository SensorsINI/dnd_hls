...
def qbuildModel(resize, hidden,bits):
    ...
    if resize > 0:
        ...
        x = QDense(1, kernel_quantizer=quantized_bits(bits),
           bias_quantizer=quantized_bits(bits),
           name="fc2")(x)
        x = QActivation("quantized_bits(16)", name="doutput")(x)
...
def trainFunction(traineventfile, testeventfile,  trainbatches, testbatches,resize, hidden, epochs, repeat, mtype, Train,wkfiles,bits):
    ...
    if Train:
        prefix = '0308split82q2x8bitaw' + str(repeat) + 'MSEO1' + middlefix + str(hidden) + '_linear_' + str(resize)
    else:
        prefix = '0308split82q2x8bitaw' + str(repeat) + 'MSEO1' + middlefix + str(hidden) + '_linear_' + str(resize)
    ...
    if Train:
        ...
        filepath='0308split82q2x8bitawawI'+str(resize) + middlefix + str(hidden) + "-{epoch:02d}-{val_accuracy:.3f}.h5"
        ...
        for wkfile in wkfiles:
            ...
            for i,(batchx,batchy) in enumerate(testgenerator):
                ...
                if icount > 0 and icount % 1000 == 0:
                    ...
                    plot_roc_curve(y_true,initpredictions,wkfile.replace('.h5','_dri_batch'+str(icount)+'_' + str("%.3f"%rocauc)))
                    np.save(str(bits)+ 'bits_dri' + '_batch'+str(icount)+'_' + str("%.3f"%rocauc)+'.npy',np.concatenate([y_true,initpredictions],axis=0))
                icount +=1
            ...
            plot_roc_curve(y_true,initpredictions,wkfile.replace('.h5','_dri_tbatch'+str(icount)+'_' + str("%.3f"%rocauc)))
            np.save(str(bits)+ 'bits_dri' + '_tbatch'+str(icount)+'_' + str("%.3f"%rocauc)+'.npy',np.concatenate([y_true,initpredictions],axis=0))
...
if bits == 32:
    ...
    # wkfiles1 = ['0308pm16bitsqsigmoidputbin100tau'+str(bits)+'bitaw0MSEO1qH20_linear_7weights.h5']
    wkfiles1 = ['4bitMLPmodel.h5']
    middle = 'qsingle'
